...
class ActionModel(nn.Module):
    ...
    Enhanced to include additional intermediate layers and a dropout mechanism
    for better generalization. We keep the same default architecture but add
    optional expansions.
    """
    def __init__(self, input_dim=23, action_dim=19, version=3, hidden_size=256):
        """
        Initializes the ActionModel with specified input and action dimensions,
        plus an optional hidden_size parameter for deeper networks.
        ...
        # You can expand this to be deeper or more complex
...
class PrioritizedReplayBuffer:
    ...
    Now includes an option to automatically remove old samples with extremely low priority
    ...
    def sample(self, batch_size, beta=0.4):
        ...
        if len(self.buffer) == 0:
            # Safety check: if there's nothing in the buffer, return dummy results
...
# Infrastructure Categories and Items (corrected bracket)
INFRASTRUCTURE = {
    "Transportation Infrastructure": {
        "Roads & Highways": [
...
class Entity:
    ...
    Now includes advanced attributes (mood, population, specialization, ambition).
    """
    def __init__(
        self, 
        name, 
        ...
        Args:
            ...
            mood (str, optional): Mood of the entity (affects actions). Defaults to "neutral".
            ambition (int, optional): Ambition level (0-100). Defaults to 50.
            population (int, optional): For multi-person entities or large groups. Defaults to 1.
            specialization (str, optional): An additional specialization for the entity. Defaults to None.
        """
        self.name = name
        self.role = role
        self.health = health
        ...
        self.mood = mood  # e.g., "happy", "neutral", "angry", "depressed"
        self.ambition = ambition  # Affects discovery, building, alliances, etc.
        self.population = population  # If this entity represents a group or a settlement
        self.specialization = specialization  # Additional secondary role/specialty
    ...
    def get_state_vector(self, global_knowledge):
        ...
        """
        # The original 23 features plus expansions for ambition, mood, population
        # We'll convert mood into a numerical index for simplicity
        mood_map = {"happy": 1.0, "neutral": 0.0, "angry": -1.0, "depressed": -0.5}
        mood_value = mood_map.get(self.mood, 0.0)
        ...
        state = [
            ...
            # Extended features
            self.ambition / 100.0,       # normalized ambition
            mood_value,                  # mood as numeric
            self.population / 1000.0,    # normalized population
        ]
        return state
    ...
    def can_discover_knowledge(self):
        """
        Determines if the entity can discover new knowledge based on role, intelligence, and ambition.
        ...
        if self.intelligence > 50:
            base_chance *= 2
        # Ambition also influences discovery
        base_chance *= (1 + (self.ambition / 200.0))  # e.g., ambition 100 doubles the chance

        return random.random() < base_chance

    def can_teach(self):
        """
        Determines if the entity can teach others based on intelligence, role, and mood.

        Returns:
            bool: True if the entity can teach, False otherwise.
        """
        if self.mood in ["angry", "depressed"]:
            # Less likely to teach if mood is negative
            return False
    ...
    def update(self, simulation_year, event_logger, global_knowledge, model, target_model, experience_container, epsilon):
        ...
        # Calculate reward based on changes (more advanced reward function)
        new_resources_sum = sum(self.resources.values())
        resource_gain = (new_resources_sum - old_resources_sum) * 0.01
        intelligence_gain = (self.intelligence - old_intelligence) * 0.1
        # If mood improved, small reward; if it worsened, negative reward
        mood_reward = 0
        if self.mood == "happy":
            mood_reward += 0.1
        elif self.mood == "angry":
            mood_reward -= 0.1
        energy_penalty = (old_energy - self.energy) * 0.05
        reward = resource_gain + intelligence_gain + mood_reward - energy_penalty
    ...
    def perform_action(self, action, simulation_year, event_logger, global_knowledge):
        ...
        if self.energy >= energy_cost:
            ...
            # Slight chance mood changes after any action (more random realism)
            self.fluctuate_mood()
        else:
            # Not enough energy to perform the action, force rest
            self.rest(simulation_year, event_logger, global_knowledge)

    def fluctuate_mood(self):
        """
        Causes mood to change spontaneously after actions. 
        Moods: "happy", "neutral", "angry", "depressed"
        """
        roll = random.random()
        if roll < 0.02:
            self.mood = "happy"
            ...
            # remain the same or revert to neutral occasionally
    ...
    def gather_resources(self, simulation_year, event_logger, global_knowledge):
        ...
        # Also, if entity is specialized or has a relevant skill
        if self.specialization in ["Farming", "Mining"]:
            gather_bonus += 1
    ...
    def build_structures(self, simulation_year, event_logger, global_knowledge):
        ...
        # If specialized in Architecture or Engineering, reduce cost
        if self.specialization in ["Architecture", "Engineering"]:
            build_cost_gold = max(10, build_cost_gold - 10)
            build_cost_food = max(10, build_cost_food - 5)
    ...
    def explore_land(self, simulation_year, event_logger, global_knowledge):
        ...
        # Higher ambition increases chance of major discovery
        if random.random() < (0.001 + self.ambition / 100000.0):
    ...
    def form_alliance(self, simulation_year, event_logger, global_knowledge):
        ...
        # For demonstration, we simulate forming an alliance with a random known name
        possible_allies = ["Entity_5", "Entity_10", "Entity_42"]
        if random.random() < 0.05:
            ally = random.choice(possible_allies)
            if ally not in self.alliances:
    ...
    def adapt_role(self, global_knowledge):
        ...
        # Example role adaptation logic based on intelligence, influence, or specialized knowledge
        if self.intelligence > 80 and "strategic_thinking" in global_knowledge:
            ...
            self.role = "Diplomat"
        # Additional advanced logic for specialized roles
        elif self.specialization in ["Farming"] and "advanced_agriculture" in global_knowledge:
            self.role = "Farmer"
    ...
    def trade_goods(self, simulation_year, event_logger, global_knowledge):
        ...
        Args:
            ...
            global_knowledge (dict): The collective knowledge.
        """
        # Duplicate method remains pass here to avoid confusion, original 'engage_in_trade' is separate
        pass
...
def initialize_world(num_entities):
    ...
    """
    world = []
    possible_specializations = ["Farming", "Mining", "Engineering", "Architecture", "Science", "Diplomacy"]
    for i in range(num_entities):
        role = random.choice(ALL_JOBS)
        # Additional random expansions for mood and ambition
        mood = random.choice(["happy", "neutral", "angry", "depressed"])
        ambition = random.randint(0, 100)
        pop = random.randint(1, 5)  # each entity may represent a small group
        specialization = random.choice(possible_specializations + [None, None])  # some will be None
        entity = Entity(
            name=f"Entity_{i}",
            role=role,
            health=100,
        ...
        with open(filename, "r") as f:
            ...
            for d in data:
                entity = Entity(
                    ...
                    specialization=d.get("specialization", None)
                )
                entity.energy = d["energy"]
                entity.max_energy = d["max_energy"]
                entity.speed = d["speed"]
                entity.direction = tuple(d["direction"])
...
def load_model(model, filename="model_weights.pt"):
    """
    Loads the model's state dictionary from a file with weights_only=True (if supported).
    ...
    if not os.path.exists(filename):
        ...
        return
    try:
        # Use weights_only=True to comply with future PyTorch versions, if available
...
def run_simulation():
    ...
    This version is heavily enhanced to provide more robust and realistic AI behavior.
    """
    if rank == 0:
        ...
        print("Now it includes advanced mood, ambition, population, specializations, and more.\n")
        print("Saved Files:")
        ...
        print("5. 'event_log.txt' - Logs major events happening in the simulation.\n")
        ...
        if reset_choice in ['yes', 'y']:
            try:
                ...
                world = initialize_world(5000)  # Keep a large number for a richer simulation
                global_knowledge = {}
        ...
        # We now consider the extended state dimension (23 + 3 new = 26). 
        # So let's update input_dim to 26 in the constructor:
        action_dim = len(ACTION_LIST)  # Ensure action_dim matches the number of actions
        model = ActionModel(input_dim=26, action_dim=action_dim)  
        target_model = ActionModel(input_dim=26, action_dim=action_dim)  
    ...
    for simulation_year in range(simulation_year_start + 1, simulation_year_end + 1):
        ...
        if rank == 0:
            ...
            if simulation_year % 100 == 0 or simulation_year == simulation_year_end:
                ...
                # Additional stats: average mood distribution, population
                moods = [e.mood for e in world]
                happy_count = moods.count("happy")
                angry_count = moods.count("angry")
                depressed_count = moods.count("depressed")
                neutral_count = moods.count("neutral")
                total_population = sum(e.population for e in world)
                ...
                print(f"          Number of Towns: {num_towns}, Number of Homes: {num_homes}, Total Population: {total_population}")
                print(f"          Mood Distribution -> Happy: {happy_count}, Angry: {angry_count}, Depressed: {depressed_count}, Neutral: {neutral_count}\n")
